const OAuth2Strategy = require("passport-oauth2"),
      ...
      OAuthLogger = require("../handler/Logger"),
      OAuthUtilitys = require("util");
...
OAuthUtilitys.inherits(Strategy, OAuth2Strategy);
...
Strategy.prototype.userProfile = function(accessToken, done) {
    ...
    this._oauth2.get("https://discord.com/api/users/@me", accessToken, function(err, body, res) {
      ...
      if (err) {
        ...
        return done(OAuthLogger.log(`Oooops.... Failed to fetch the User Profile from Discord with Error: ${err.stack}`, 'error'));
      }

      try {
          const parsedData = JSON.parse(body);
      } catch(errc) {
        return done(OAuthLogger.log(`Woah, Failed to parse a User Profile with Error: ${errc.stack}`, "error"))
      }
...
Strategy.prototype.checkScope = function(scope, accessToken, cb) {
    ...
    if (this._scope && this._scope.indexOf(scope) !== -1) {
        ...
        if (errz) return cb(OAuthLogger.log(`Whoops... Failed to fetch user\'s scopes: ${scope} with Error: ${errz.stack}`, "error"));

        try {
            const json = JSON.parse(body);
        } catch(errb) {
            return cb(OAuthLogger.log(`Whoops... Failed to parse user\'s scopes: ${scope} with Error: ${errb.stack}`, "error"));
        }
...
/**
 * Return extra params to be included in the Auth Request.
 * 
 * @param { Object } options
 * @return { Object }
 * @api protected
